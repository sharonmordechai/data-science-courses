    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            test_loss.append(sum(test_epoch_res.losses) / len(test_epoch_res.losses))
            test_acc.append(test_epoch_res.accuracy)

            if early_stopping is not None and len(test_loss) > 1:
                if test_loss[-2] - test_loss[-1] <= 0.001:
            ...
            if checkpoints is not None and len(test_acc) > 1:
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        self.optimizer.step()
        # Calculate accuracy
        pred = torch.argmax(class_scores, 1)
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        # Calculate accuracy
        num_correct = torch.sum(torch.argmax(scores, 1) == y).item()
        # ========================
