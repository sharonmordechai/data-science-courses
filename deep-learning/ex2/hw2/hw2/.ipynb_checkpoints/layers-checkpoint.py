...
class LeakyReLU(Layer):
	...
	def backward(self, dout):
		...
		# ====== YOUR CODE: ======
		# inter =
		dx = dout * torch.where(x > 0, torch.Tensor([1.]), torch.Tensor([self.alpha]))
		# ========================
...
class Linear(Layer):
	...
	def backward(self, dout):
		...
		dx = dout @ self.w
		self.dw +=  dout.T @ x
		self.db += dout.T @ torch.ones(x.shape[0])
		# ========================
...
class Dropout(Layer):
	...
	def forward(self, x, **kw):
		...
		if self.training_mode:
			dropout_mask = torch.zeros_like(x) + (1 - self.p)
			dropout_mask = torch.bernoulli(dropout_mask)
			out = x * dropout_mask
...
class MLP(Layer):
	...
	def __init__(
		...
		**kw,
		...
		for curr_num_features in hidden_features:
			layers.append(Linear(prev_num_features, curr_num_features))
			layers.append(activation_cls())
			if activation.lower() == "relu" and dropout > 0:
				layers.append(Dropout(dropout))
			prev_num_features = curr_num_features
		layers.append(Linear(prev_num_features, num_classes))
		# ========================
