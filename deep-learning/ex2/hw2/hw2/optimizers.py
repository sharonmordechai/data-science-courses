...
class MomentumSGD(Optimizer):
    ...
    def step(self):
        for (p, dp), v_index in zip(self.params, range(len(self.params))):
            if dp is None:
                continue
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            self.velocity[v_index] *= self.momentum
            self.velocity[v_index] -= self.learn_rate * dp
            p += self.velocity[v_index]
            # ========================
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for (p, dp), r_index in zip(self.params, range(len(self.params))):
            if dp is None:
                continue
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            self.r[r_index] *= self.decay
            self.r[r_index] += (1 - self.decay) * (dp ** 2)
            p -= (self.learn_rate / (torch.sqrt(self.r[r_index] + self.eps))) * dp
