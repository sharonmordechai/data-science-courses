...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        y_output, h = self.model.forward(x, self.h)
        ...
        for i in range(seq_len):
            loss += self.loss_fn(y_output[:,i,:], y[:,i])
        # Backward pass
        loss.backward()

        self.optimizer.step()
        y_output = torch.argmax(y_output, dim=-1)
        num_correct = torch.sum(y_output == y).float()
        self.h = h.detach()
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            # ====== YOUR CODE: ======
            # Forward pass
            y_output, self.h = self.model.forward(x, self.h)
