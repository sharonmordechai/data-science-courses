...
def char_maps(text: str):
    ...
    char_to_idx = {chars[i]:idxs[i] for i in range(len(idxs))}
...
def remove_chars(text: str, chars_to_remove):
    ...
    n_removed = initial_len - len(text_clean)
    # ========================
...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    """
    # ====== YOUR CODE: ======
    idxs = torch.Tensor([char_to_idx[l] for l in list(text)]).to(torch.int64)
    result = torch.nn.functional.one_hot(idxs, num_classes=len(char_to_idx.keys())).to(torch.int8)
    # ========================
    return result
...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    """
    # ====== YOUR CODE: ======
    txt = torch.argmax(embedded_text, dim=1)
    lst = [idx_to_char[i.item()] for i in txt]
    result = ''.join(lst)
    # ========================
    return result
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    # Embed the given text, excluding the last character
    ...
    chars = chars[1:N * S + 1]
    labels = torch.Tensor([char_to_idx[char] for char in chars])
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        ...
        embedded_text = chars_to_onehot(start_sequence, char_to_idx).unsqueeze(0).type(torch.float).to(device)
        h = None
        while len(out_text) < n_chars:
            y, h = model(embedded_text, h)
            y = hot_softmax(y[:, -1, :], -1, temperature=T)
            y = torch.multinomial(y, num_samples=1)

            y = idx_to_char[y.item()]
            out_text += y

            embedded_text = chars_to_onehot(y, char_to_idx).unsqueeze(0).type(torch.float).to(device)
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        # ====== YOUR CODE: ======
        num_batches = len(self.dataset)//self.batch_size #int(torch.ceil(torch.Tensor([])).item())
        max_num = num_batches*self.batch_size
        ranges = [range(i, max_num, num_batches) for i in range(num_batches)]
        idx = []
        for j in ranges:
            idx.extend(list(j))
        # ========================
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i in range(n_layers):
            ...
            self.add_module(f'W_xz_{str(i)}', self.layer_params[-1][-1])

            self.layer_params[-1].append(nn.Linear(h_dim, h_dim, bias=True))
            self.add_module(f'W_hz_{str(i)}', self.layer_params[-1][-1])

            self.layer_params[-1].append(nn.Linear(in_dim, h_dim, bias=False))
            self.add_module(f'W_xr_{str(i)}', self.layer_params[-1][-1])

            self.layer_params[-1].append(nn.Linear(h_dim, h_dim, bias=True))
            self.add_module(f'W_hr_{str(i)}', self.layer_params[-1][-1])

            self.layer_params[-1].append(nn.Linear(in_dim, h_dim, bias=False))
            self.add_module(f'W_xg_{str(i)}', self.layer_params[-1][-1])

            self.layer_params[-1].append(nn.Linear(h_dim, h_dim, bias=True))
            self.add_module(f'W_hg_{str(i)}', self.layer_params[-1][-1])

            self.layer_params[-1].append(nn.Dropout(dropout))
            self.add_module(f'dropout_{str(i)}', self.layer_params[-1][-1])

            in_dim = h_dim

        self.fc_y = nn.Linear(h_dim, out_dim, bias=True)
        self.add_module('W_hy', self.fc_y)
        # ========================
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        for t in range(seq_len):
            prev_h = torch.stack(layer_states, dim=1)
            layer_states = []
            x = layer_input[:, t, :]

            for layer_i, layer_params in zip(range(self.n_layers), self.layer_params):
                h_k = prev_h[:, layer_i,:]
                W_xz, W_hz, W_xr, W_hr, W_xg, W_hg, dropout = layer_params
                z = torch.sigmoid(W_xz(x) + W_hz(h_k))
                r = torch.sigmoid(W_xr(x) + W_hr(h_k))
                g = torch.tanh(W_xg(x) + W_hg(h_k * r))
                new_h = z * h_k + (1 - z) * g
                x = dropout(new_h.clone())
                layer_states.append(new_h)

            layer_output.append(self.W_hy(x))
