    ...
    def train(
        ...
        for epoch_idx in range(max_epochs):
            ...
            for x_train, y_train in dl_train:                           
                ...
                average_loss += loss_fn(x_train, y_train, x_scores, y_pred)
                total_correct += (y_train == y_pred).sum().item()
                grad = loss_fn.grad() + weight_decay * self.weights
                self.weights = self.weights - learn_rate * grad
                count_batches += 1
            
            train_res.loss.append(average_loss / count_batches)
            train_res.accuracy.append(total_correct / (count_batches * dl_train.batch_size))            
            
            total_correct = 0
            average_loss = 0
            count_batches = 0
            for x_val, y_val in dl_valid:                           
                y_pred, x_scores = self.predict(x_val)
                average_loss += loss_fn(x_val, y_val, x_scores, y_pred)
                total_correct += (y_val == y_pred).sum().item()
                count_batches += 1
            
            valid_res.loss.append(average_loss / count_batches)
            valid_res.accuracy.append(total_correct / (count_batches * dl_valid.batch_size)) 
            # ========================
            print(".", end="")
