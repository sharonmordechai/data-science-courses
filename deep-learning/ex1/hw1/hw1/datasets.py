...
def random_labelled_image(
    ...
    label = torch.randint(0, num_classes, size=(1,), dtype=dtype)[0]
    # ========================
...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        # ====== YOUR CODE: ======
        if index >= len(self):
            raise ValueError(f'{self.__class__.__name__} index out of range')
...
class ImageStreamDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        # ====== YOUR CODE: ======
        while True:
            yield iter(random_labelled_image(self.image_dim, self.num_classes))
        # ========================
