...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            dist_arr = dist_matrix[:,i].reshape(-1,1)
            neighbors = np.concatenate((dist_arr, self.y_train.reshape(-1, 1)), axis = 1) 
            neighbors_sorted = neighbors[neighbors[:, 0].argsort()]
            k_neighbors = neighbors_sorted[:self.k]
            common_classes = np.unique(k_neighbors[:, 1], return_counts=True)
            target_common_class = common_classes[0][common_classes[1].argmax()]
            y_pred = np.append(y_pred, target_common_class)
            # ========================
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    dists = x1_norm + x2_norm - 2.0 * torch.mm(x1, torch.transpose(x2, 0, 1))
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for i in range(num_folds):
            train_data, val_data = dataloaders.create_train_validation_loaders(ds_train, validation_ratio=1.0/num_folds)
            model.train(train_data)
            x_val, y_val = dataloader_utils.flatten(val_data)
            y_pred = model.predict(x_val)
            accuracy_list.append(accuracy(y_val, y_pred))            
