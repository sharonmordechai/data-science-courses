...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        # for the encoder
        self.mu_alpha = nn.Linear(self.n_features, z_dim)
        self.log_sigma2_alpha = nn.Linear(self.n_features, z_dim)
        
        # for the decoder
        self.mu_beta = nn.Linear(z_dim, self.n_features)
        # ========================
    ...
    def encode(self, x):
        ...
        mu = self.mu_alpha(h)
        log_sigma2 = self.log_sigma2_alpha(h)
        
        # Reparametrization trick: Sample u from an isotropic gaussian and use it to create z.
        u = torch.randn_like(mu)
        z = mu + u * (log_sigma2.mul(0.5).exp_())
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        h_rec = self.mu_beta(z)
        h_rec = h_rec.view([-1, self.features_shape[0], self.features_shape[1], self.features_shape[2]])
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    dz = z_mu.size(1)
    log_det = torch.log(torch.prod(torch.exp(z_log_sigma2), dim=1))
    tr_z_sigma = torch.sum(torch.exp(z_log_sigma2), 1)
    
    data_loss = (1 / (x_sigma2 * dx)) * (torch.norm(x - xr, dim=1)**2)
    data_loss = torch.mean(data_loss)
    
    kldiv_loss = tr_z_sigma + torch.norm(z_mu, dim=1)**2 - dz - log_det
